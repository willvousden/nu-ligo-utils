...
nparams_time_phase_marginalized = 7
params_time_phase_marginalized_dtype = [('log_mc', np.float),
                                        ('eta', np.float),
                                        ('cos_iota', np.float),
                                        ('psi', np.float),
                                        ('ra', np.float),
                                        ('sin_dec', np.float),
                                        ('log_dist', np.float)]
params_time_phase_marginalized_latex = [r'\log \mathcal{M}',
                                        r'\eta',
                                        r'\cos(\iota)',
                                        r'\psi',
                                        r'\alpha',
...
def to_time_phase_marginalized_params(arr):
    """Returns a view of ``arr`` with labeled columns."""
    return arr.view(params_time_phase_marginalized_dtype)

def from_time_phase_marginalized_params(arr):
    """Returns a view of ``arr`` as a float array, assuming it is of
    ``params_time_marginalized_dtype``."""

    shape = arr.shape

    return arr.view(float).reshape(shape + (nparams_time_phase_marginalized,))

def params_to_time_phase_marginalized_params(arr):
    arr = to_params(arr)
    shape = arr.shape

    arr_tm = np.zeros(shape, dtype=params_time_phase_marginalized_dtype)

    for n,t in params_time_phase_marginalized_dtype:
        arr_tm[n] = arr[n]

    return arr_tm

def time_phase_marginalized_params_to_params(arr, time=0, phi=np.pi/2.0):
    """Returns an array of params corresponding to ``arr`` with the
    ``time`` column added and assigned the value in ``time``.
    """
    arr = to_time_phase_marginalized_params(arr)
